...
The working directory for this project is the same folder the README you are reading right now is located. All commands assume you are in the `{PROJECT_ROOT}/apps/{{appName}}` in your console.
...
From `apps/{{appName}}` you can run the following command to validate your environment:
...
```shell
DATABASE_URL="mysql://root@localhost:{{databasePort}}/{{appName}}"
```
...
The app will be served at `http://localhost:{{devPort}}`.
...
This starts the MySQL container (running on port `{{databasePort}}`) and applies any schema changes as needed.

The first time you run this command it will seed the database with the contents of `apps/{{appName}}/seed_data`
...
If you've just installed the Stripe CLI (or just been granted access to this Stripe account), you'll need to login via the commandline. Run `stripe login` and follow the prompts to connect your `stripe` CLI to the {{projectTitle}} Stripe account. To confirm your connection, you should see a _Restricted Keys_ entry for your machine in the _Developers_ > _API Keys_ section of the Stripe Dashboard.
...
The `dev:stripe` node script is a shorthand for `stripe listen --forward-to localhost:{{devPort}}/api/skill/webhook/stripe`.
...
```bash
pscale connect {{appName}} PLANETSCALE_DB_BRANCH_NAME --port {{databasePort}}
```
...
First, make sure Docker is running with MySQL at the appropriate port (`{{databasePort}}`).
...
```bash
pscale branch create {{appName}} PLANETSCALE_DB_BRANCH_NAME
```
...
With a connection open at port {{databasePort}}, we can now push our schema changes up to Planetscale.
...
```bash
pscale deploy-request create {{appName}} descriptive-branch-name
```
...
```bash
pscale database restore-dump {{appName}} next-steps --dir ./seed_data
```
...
```bash
pscale database dump {{appName}} main --output ./seed_data/pscale_data_dump
```

Now the `seed_data/pscale_data_dump` folder holds a database dump you can restore to your branch:

```bash
pscale database restore-dump {{appName}} next-steps --dir ./seed_data/pscale_data_dump --overwrite-tables
```
