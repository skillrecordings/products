...
const importCourseData = async () => {
  ...
  // TODO: Remove all of this once ready to run the full script (need to get Mux assets uploaded first)
  console.log('Products parsed, time to leave')
  if (Boolean(products)) {
    process.exit()
  }
  ...
  for (const entry of muxPlaybackEntries) {
    ...
    const {transcript: _transcript, subtitles_url} = uniqueLessons[lessonSlug]
