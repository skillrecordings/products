...
export type IngestEvents = {
  ...
  'article/clear.scheduled': ArticleClearScheduled
}
...
function isNowOrPast(isoDateTime: string | null) {
...
const checkArticlePublishState = inngest.createFunction(
  ...
  async ({event, step}) => {
    const articleState = await step.run('Load Article Status', async () => {
      const article = await getArticle(event.data.slug.current)
      return article.state
    })

    if (articleState !== 'published') {
      let publishAtDate = await step.run('Load Article Status', async () => {
        const article = await getArticle(event.data.slug.current)
      ...
      if (publishAtDate) {
        ...
        if (isNowOrPast(publishAtDate)) {
          await sanityWriteClient.patch(event.data._id).set({
            state: 'published',
          })
        }
      }
