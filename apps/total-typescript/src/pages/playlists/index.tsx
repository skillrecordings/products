...
const PlaylistsPage: React.FC<{playlists: SanityDocument[]}> = ({
  ...
  return (
    <Layout
      meta={{
        title: `TypeScript Playlists from Matt Pocock`,
        description: `TypeScript playlists by Matt Pocock that will help you learn how to use TypeScript as a professional web developer through exercise driven examples.`,
        ogImage: {
    ...
    >
      <main className="relative z-10 flex flex-col items-center justify-center py-32 sm:py-40">
        <h1 className="px-5 text-center font-heading text-5xl font-bold sm:text-5xl">
          TypeScript Video Playlists
        </h1>
        <p className="max-w-lg px-5 pt-8 text-center text-lg text-rose-100/90">
          <Balancer>
            A collection of playlists featuring TypeScript videos for you to use
            on your journey to TypeScript wizardry.
          </Balancer>
        </p>
        {playlists && (
          <ul className="flex max-w-screen-md flex-col gap-5 px-5 pt-10 sm:gap-8 sm:pt-20">
            {playlists.map(({title, slug, image, description, lessons}, i) => {
              return (
                <li
                  key={slug.current}
