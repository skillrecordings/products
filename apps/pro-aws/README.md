...
The working directory for this project is the same folder the README you are reading right now is located. All commands assume you are in the `{PROJECT_ROOT}/apps/pro-aws` in your console.
...
From `apps/pro-aws` you can run the following command to validate your environment:
...
```shell
DATABASE_URL="mysql://root@localhost:3309/pro-aws"
```
...
The first time you run this command it will seed the database with the contents of `apps/pro-aws/seed_data`
...
```bash
pscale connect pro-aws BRANCH_NAME --port 3309
```
...
```bash
pscale connect pro-aws PLANETSCALE_DB_BRANCH_NAME --port 3309
```
...
```bash
pscale branch create pro-aws PLANETSCALE_DB_BRANCH_NAME
```
...
```bash
pscale deploy-request create pro-aws descriptive-branch-name
```
...
```bash
pscale database restore-dump pro-aws next-steps --dir ./seed_data
```
...
```bash
pscale database dump pro-aws main --output ./seed_data/pscale_data_dump
```

Now the `seed_data/pscale_data_dump` folder holds a database dump you can restore to your branch:

```bash
pscale database restore-dump pro-aws next-steps --dir ./seed_data/pscale_data_dump --overwrite-tables
```

## Edit content

Edit workshops with Sanity at [pro-aws.sanity.studio](https://pro-aws.sanity.studio/).
