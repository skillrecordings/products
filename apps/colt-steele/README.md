...
The working directory for this project is the same folder the README you are reading right now is located. All commands assume you are in the `{PROJECT_ROOT}/apps/colt-steele` in your console.
...
From `apps/colt-steele` you can run the following command to validate your environment:
...
```shell
DATABASE_URL="mysql://root@localhost:3309/colt-steele"
```
...
The app will be served at `http://localhost:3033`.
...
The first time you run this command it will seed the database with the contents of `apps/colt-steele/seed_data`
...
```bash
pscale connect colt-steele BRANCH_NAME --port 3309
```
...
If you've just installed the Stripe CLI (or just been granted access to this Stripe account), you'll need to login via the commandline. Run `stripe login` and follow the prompts to connect your `stripe` CLI to the Colt Steele Stripe account. To confirm your connection, you should see a _Restricted Keys_ entry for your machine in the _Developers_ > _API Keys_ section of the Stripe Dashboard.
...
The `dev:stripe` node script is a shorthand for `stripe listen --forward-to localhost:3033/api/skill/webhook/stripe`.
...
```bash
pscale connect colt-steele PLANETSCALE_DB_BRANCH_NAME --port 3309
```
...
```bash
pscale branch create colt-steele PLANETSCALE_DB_BRANCH_NAME
```
...
```bash
pscale deploy-request create colt-steele descriptive-branch-name
```
...
```bash
pscale database restore-dump colt-steele next-steps --dir ./seed_data
```
...
```bash
pscale database dump colt-steele main --output ./seed_data/pscale_data_dump
```

Now the `seed_data/pscale_data_dump` folder holds a database dump you can restore to your branch:

```bash
pscale database restore-dump colt-steele next-steps --dir ./seed_data/pscale_data_dump --overwrite-tables
```

## Edit content

Edit workshops with Sanity at [colt-steele.sanity.studio](https://colt-steele.sanity.studio/).
