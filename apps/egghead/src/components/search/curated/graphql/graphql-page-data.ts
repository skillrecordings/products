const graphqlPageData = [
  {
    id: 'get-started',
    title: 'Up and Running',
    name: 'Get Started',
    resources: [
      {
        title:
          'Build a full-stack JAMstack application with Gatsby, Hasura, and Auth0',
        path: '/playlists/build-a-full-stack-jamstack-application-with-gatsby-c6f8',
        image:
          'https://d2eip9sf3oo6c2.cloudfront.net/tags/images/000/001/034/full/graphqllogo.png',
        byline: 'Kyle Gill',
      },
      {
        title: 'Getting Started With Hasura and GraphQL',
        path: '/playlists/getting-started-with-hasura-cloud-872e',
        image:
          'https://d2eip9sf3oo6c2.cloudfront.net/tags/images/000/001/034/full/graphqllogo.png',
        byline: 'Ali Spittel',
      },
      {
        title: 'Learn Apollo Client Hooks',
        path: '/playlists/learn-apollo-client-hooks-9226f672',
        image:
          'https://d2eip9sf3oo6c2.cloudfront.net/tags/images/000/000/026/full/react.png',
        byline: 'Tomasz Łakomy',
      },
    ],
  },
  {
    id: 'project-based',
    title: 'For Your Porfolio',
    name: 'Project-Based',
    resources: [
      {
        title:
          'Building a Serverless JAMStack Todo app with Netlify, Gatsby, GraphQL, and FaunaDB',
        path: '/playlists/building-a-serverless-jamstack-todo-app-with-netlify-gatsby-graphql-and-faunadb-53bb',
        image:
          'https://d2eip9sf3oo6c2.cloudfront.net/tags/images/000/001/281/full/Fauna_Logo_blue.png',
        byline: 'Chris Biscardi',
      },
      {
        title: 'React Real-Time Messaging with GraphQL using urql and OneGraph',
        path: '/playlists/react-real-time-messaging-with-graphql-using-urql-and-onegraph-be5a',
        image:
          'https://d2eip9sf3oo6c2.cloudfront.net/playlists/square_covers/000/409/013/full/EGH_RealtimeGraphqL.png',
        byline: 'Ian Jones',
      },
      {
        title: 'Build a CRUD-ready GraphQL API + Admin UI with KeystoneJS',
        path: '/playlists/build-a-crud-ready-graphql-api-admin-ui-with-keystonejs-86fa4eea',
        image:
          'https://d2eip9sf3oo6c2.cloudfront.net/tags/images/000/001/034/full/graphqllogo.png',
        byline: 'Simon Vrachliotis',
      },
    ],
  },
  {
    id: 'graphql-featured',
    title: 'Featured',
    resources: [
      {
        name: 'Beginner',
        title: 'GraphQL Query Language',
        path: '/courses/graphql-query-language',
        image:
          'https://d2eip9sf3oo6c2.cloudfront.net/series/square_covers/000/000/236/full/EGH_GraphQLQuery_Final.png',
        byline: 'Eve Porcello',
      },
      {
        name: 'Intermediate',
        title:
          'WordPress as a Headless Content Management System (CMS) and GraphQL API',
        path: '/playlists/headless-wordpress-4a14',
        image:
          'https://d2eip9sf3oo6c2.cloudfront.net/playlists/square_covers/000/410/100/full/Headless-Wordpress.png',
        byline: 'Kevin Cunningham',
      },
      {
        name: 'Advanced',
        title:
          'Scalable Offline-Ready GraphQL Applications with AWS AppSync & React',
        path: '/courses/scalable-offline-ready-graphql-applications-with-aws-appsync-react',
        image:
          'https://d2eip9sf3oo6c2.cloudfront.net/series/square_covers/000/000/211/full/EGH_GraphQL-AWs_Final.png',
        byline: 'Nader Dabit',
      },
    ],
  },
  {
    id: 'graphql-video',
    name: 'Secure GraphQL',
    title: 'Securing GraphQL Backends with JWTs',
    description: `Demystify auth while learning how to use JSON Web Tokens (JWTs) with GraphQL APIs!`,
    byline: 'Sam Julien',
    path: 'talks/graphql-securing-graphql-backends-with-jwts',
    image:
      'https://res.cloudinary.com/dg3gyk0gu/image/upload/v1618961127/egghead%20talks/graphql-securing-graphql-backends-with-jwts.png',
  },
  {
    id: 'graphql-serverless',
    title: 'Going Serverless with GraphQL',
    name: 'Serverless Architecture',
    resources: [
      {
        title:
          'Scalable Offline-Ready GraphQL Applications with AWS AppSync & React',
        path: '/courses/scalable-offline-ready-graphql-applications-with-aws-appsync-react',
        image:
          'https://d2eip9sf3oo6c2.cloudfront.net/series/square_covers/000/000/211/full/EGH_GraphQL-AWs_Final.png',
        byline: 'Nader Dabit',
      },
      {
        title: 'GraphQL Data in React with Apollo Client',
        path: '/courses/graphql-data-in-react-with-apollo-client',
        image:
          'https://d2eip9sf3oo6c2.cloudfront.net/series/square_covers/000/000/231/full/EGH_Apollo-GraphQL-React_Final.png',
        byline: 'Nik Graf',
      },
      {
        title: 'Building Serverless Web Applications with React & AWS Amplify',
        path: '/courses/building-serverless-web-applications-with-react-aws-amplify',
        image:
          'https://d2eip9sf3oo6c2.cloudfront.net/series/square_covers/000/000/217/full/EGH_AmplifyAWS_React_Final.png',
        byline: 'Nader Dabit',
      },
    ],
  },
]

export default graphqlPageData
