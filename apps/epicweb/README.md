...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create epicweb-1cb1
  fly apps create epicweb-1cb1-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app epicweb-1cb1
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app epicweb-1cb1-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'epicweb-1cb1-staging' does not match app name in config file 'epicweb-1cb1'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name epicweb-1cb1-db
  fly postgres attach --postgres-app epicweb-1cb1-db --app epicweb-1cb1

  fly postgres create --name epicweb-1cb1-staging-db
  fly postgres attach --postgres-app epicweb-1cb1-staging-db --app epicweb-1cb1-staging
  ```
