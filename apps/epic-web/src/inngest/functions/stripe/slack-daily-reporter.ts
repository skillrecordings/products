...
export const slackDailyReporter = inngest.createFunction(
  ...
  async ({step}) => {
    const allTransactions: SimplifiedBalanceTransaction[] = []
    const allCharges: SimplifiedCharge[] = []
    let hasMoreTransactions = true
    let hasMoreCharges = true
    let startingAfterTransaction: string | undefined = undefined
    let startingAfterCharge: string | undefined = undefined

    while (hasMoreTransactions || hasMoreCharges) {
      const {
        transactions,
        charges,
        has_more,
        next_page_cursor_transaction,
        next_page_cursor_charge,
      }: {
        ...
        `fetch-combined-financial-data-${
          startingAfterTransaction || 'initial'
        }-${startingAfterCharge || 'initial'}`,
        () =>
          fetchCombinedFinancialData({
            range: 'this-year',
            starting_after_transaction: startingAfterTransaction,
            starting_after_charge: startingAfterCharge,
          }),
      )
      ...
      hasMoreTransactions = !!next_page_cursor_transaction
      hasMoreCharges = !!next_page_cursor_charge

      startingAfterTransaction = next_page_cursor_transaction || undefined
      startingAfterCharge = next_page_cursor_charge || undefined

      // If we've fetched all transactions but not all charges (or vice versa),
      // we need to continue fetching only the remaining data type
      if (!hasMoreTransactions && hasMoreCharges) {
        startingAfterTransaction = undefined
      } else if (hasMoreTransactions && !hasMoreCharges) {
        startingAfterCharge = undefined
      }

      // Break the loop if we've fetched all data for both types
      if (!hasMoreTransactions && !hasMoreCharges) {
        break
      }
    }

    const chargeAmountTransactions = await step.run(
    ...
    const refundAmountTransactions = await step.run(
      'calculate-refund-amount',
      async () => {
        const totalRefund = allTransactions.reduce((sum, transaction) => {
    ...
    const nicollFeeTransactions = await step.run(
      'calculate-nicoll-fee',
      async () => {
    ...
    const nicollNetTransactions = await step.run(
      'calculate-nicoll-net',
      async () => {
