...
The working directory for this project is the same folder the README you are reading right now is located. All commands assume you are in the `{PROJECT_ROOT}/apps/testing-javascript` in your console.
...
From `apps/testing-javascript` you can run the following command to validate your environment:
...
If you've just installed the Stripe CLI (or just been granted access to this Stripe account), you'll need to login via the commandline. Run `stripe login` and follow the prompts to connect your `stripe` CLI to the Testing JavaScript Stripe account. To confirm your connection, you should see a _Restricted Keys_ entry for your machine in the _Developers_ > _API Keys_ section of the Stripe Dashboard.
...
The `dev:stripe` node script is a shorthand for `stripe listen --forward-to localhost:3018/api/skill/webhook/stripe`.
...
```bash
pscale branch create testing-javascript PLANETSCALE_DB_BRANCH_NAME
```
...
```bash
pscale deploy-request create testing-javascript descriptive-branch-name
```
...
```bash
pscale database restore-dump testing-javascript next-steps --dir ./seed_data
```
...
```bash
pscale database dump testing-javascript main --output ./seed_data/pscale_data_dump
```

Now the `seed_data/pscale_data_dump` folder holds a database dump you can restore to your branch:

```bash
pscale database restore-dump testing-javascript next-steps --dir ./seed_data/pscale_data_dump --overwrite-tables
```

## Edit content

Edit workshops with Sanity at [testing-javascript.sanity.studio](https://testing-javascript.sanity.studio/).
