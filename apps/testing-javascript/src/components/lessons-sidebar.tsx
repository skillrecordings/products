...
const LessonsSidebar: React.FC<any> = ({lesson, module}) => {
  ...
  return (
    <div className="min-h-[450px] border border-black/[.08] w-full flex flex-col">
      <div className="flex p-3 border-b border-black/[.08] shrink-0">
        <Link
      ...
      <div className="grow relative">
        <ul className="absolute inset-0 overflow-y-scroll divide-y">
            ...
            return (
              <li key={item.slug}>
                <Link
                  ...
                  className={cx(
                    'flex px-3 py-2 text-sm items-baseline duration-100 hover:bg-gray-100',
                    {'bg-gray-100': item.slug === lesson.slug},
                  )}
                >
