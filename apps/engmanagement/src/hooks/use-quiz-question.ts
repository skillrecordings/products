...
function useQuestion(question: Question) {
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      answer: correct
        ...
        : Yup.string().nullable().required(`Can't stay empty. Mind to elaborate? :)`),
  ...
  const answeredCorrectly = (): boolean => {
    const allCorrect: any =
      isArray(answer?.answer) && every(answer.answer.map((a: string) => correct?.includes(a)))

    return isAnswered && hasMultipleCorrectAnswers ? allCorrect : correct === answer?.answer
  }
