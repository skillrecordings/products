...
const ThanksPage = ({ displayEmail }: any) => {
  const emailText = displayEmail && displayEmail !== 'undefined' ? `**${displayEmail}**` : ''
  const instructionText = `# Thank you for purchasing ${
    config.defaultTitle
  }. Please check your inbox.
${emailText ? `## ${emailText}` : ''}
As a final step to access the course you need
to check your inbox ${emailText ? `(${emailText})` : ''} where you will find an email from \`${
    ...
    <>
      ...
      <div className="py-8">
        <div className="pb-4 font-semibold">
          Please consider telling your friends about {config.siteUrl}, it would help me to get a
          word out. :)
        </div>
        <a
          href={tweet}
        ...
        >
          <TwitterIcon /> <span className="pl-2 font-medium">Share with your friends!</span>
        </a>
      </div>
    </>
  )
