...
const EssayQuestion: FunctionComponent<{
  ...
  const { formik, onAnswer, isAnswered, answeredCorrectly, isSubmitting } = useQuestion(question)

  return (
    <form onSubmit={onAnswer} className="w-full">
      <legend className="lg:text-4xl sm:text-3xl text-2xl font-semibold pb-6">
        <Markdown className="prose sm:prose-xl prose-lg" children={question?.question} />
      </legend>
      <label>
        <span className="text-xl font-medium pb-2 inline-block text-gray-800">Please explain:</span>
        <textarea
          disabled={isAnswered}
          name="answer"
      ...
      {isAnswered && question?.answer && (
        <Markdown children={question.answer} className="prose sm:prose-xl prose-lg pt-5" />
      )}
