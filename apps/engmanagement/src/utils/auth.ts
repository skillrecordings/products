...
export default class Auth {
  ...
  becomeUser(email: any, accessToken: any) {
    ...
    return http
      ...
      .then(({ data }) => {
        const expiresAt = JSON.stringify(data.access_token.expires_in * 1000 + new Date().getTime())
  ...
  requestSignInEmail(email: string) {
    return http.post(`${process.env.NEXT_PUBLIC_AUTH_DOMAIN}/api/v1/users/send_token`, {
      email,
    ...
    return new Promise((resolve, reject) => {
      ...
      if (typeof window !== 'undefined') {
        ...
        this.eggheadAuth.token.getToken(uri).then(
          (authResult) => {
            const user = this.handleNewSession(authResult.accessToken, authResult.data.expires_in)
            resolve(user)
          },
          (error) => {
            console.error(error)
