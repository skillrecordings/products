...
export default async function fetchConvertkitSubscriberFromServerCookie(header: string) {
  if (!process.env.CONVERTKIT_API_SECRET) throw new Error('No Convertkit Secret Key Found')
  ...
  if (!convertkitId) {
    ...
    if (isEmpty(eggheadUser)) throw new Error('unable to load convertkit subscriber')
    ...
    subscriber = await convertkitAxios
      .get(`/subscribers/${convertkitId}?api_secret=${process.env.CONVERTKIT_API_SECRET}`)
      .then(({ data }) => data.subscriber)
  }

  if (isEmpty(subscriber)) throw new Error('no convertkit subscriber was loaded')
